          ...
          @endif
            ...
            @auth
                @if(Auth::user()->id == $lost->id_user_report)
                    @if($lost->status == 1)
                    <a href="https://radi.pet/losts/poster?id={{$lost->setHiddenId()}}" target="_blank" class="text-white my-6 bg-blue-800 px-4 py-2 rounded-md font-bold text-xl inline-flex items-center justify-center w-full mb-2 btn btn-primary btn-lg sm:w-auto sm:mb-0" ><i class="fa-solid fa-download mr-4"></i> Descargar afiche</button>
